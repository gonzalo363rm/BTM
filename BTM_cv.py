...
parameters = {'T': [5, 10, 20, 30], 'M': [10, 20, 30], 'alpha': [0.01, 0.05, 0.1, 0.5, 3.3, 5.0], 'beta': [0.005, 0.01, 0.05, 0.1, 0.5], 'iterations': [5, 10, 25]}
        ...
        for alpha in parameters['alpha']:
            for beta in parameters['beta']:
                for iterations in parameters['iterations']:
                    ...
                    # coherence = btm.coherence(model.matrix_topics_words_, X, M=30)
                    grid_search_results.append({'T': T, 'M': M, 'alpha': alpha, 'beta': beta, 'iterations': iterations, 'coherence': np.mean(model.coherence_)})
...
model = btm.BTM(
    X, vocabulary, seed=12321, T=best_params['T'], M=best_params['M'], alpha=best_params['alpha'], beta=best_params['beta'])
model.fit(biterms, iterations=best_params['iterations'])
